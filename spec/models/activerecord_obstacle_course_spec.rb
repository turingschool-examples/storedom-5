...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds multiple orders by id' do
    ...
    # orders = Order.includes('id == ids')
    # orders = Order.find_by(id)
  ...
  it "groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # grouped_items = Order.select("order.*, item.*").order(items: items.name)
    grouped_items = Order.order(:item)
    # grouped_items = Order.where(id: true)
