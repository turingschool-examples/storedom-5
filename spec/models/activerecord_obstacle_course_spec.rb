...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'returns the sorted names of items for a user order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where(user_id: 3)[2].items.order(:id).pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    order_item_names = Item.joins(:orders).distinct.pluck(:name)
  ...
  it 'returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.select("name, count(user_id) as total_order_count").joins(:orders).group(:name)
    # ---------------------------------------------------------------
