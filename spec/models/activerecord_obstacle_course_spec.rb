...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:items).select(:name).where('items.id = ?', item_8.id).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns the sorted names of items for a user order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where(user_id: 3).limit(1).offset(2).first.items.order(:id).pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Item.find(4).orders.where(user_id: user_2.id)
    # -----------------------------------------------------------
