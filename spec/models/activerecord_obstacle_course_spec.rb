...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'returns the sorted names of items for a user order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    items_for_user_3_third_order = User.find(3).orders[2].items.distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    orders = Order.joins(:order_items)
                  .where(user: user_2, "order_items.item_id": item_4.id)
    # -----------------------------------------------------------
  ...
  it 'returns items that are associated with one or more orders' do
    ...
    # Solution goes here
    ordered_items = Item.where.not(name: 'Unordered Item').compact
    # ---------------------------------------------------------------
