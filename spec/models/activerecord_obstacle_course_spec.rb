...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'should return all items except items: 3, 4 & 5' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    ids_not_included = [3,4,5]

    items = Item.where.not(id: ids_not_included).order(:id)
    # ------------------------------------------------------------
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.where(user_id: 2).joins(:items).where(:items => {id: 4} )
    # -----------------------------------------------------------
  ...
  it 'returns a table of information for all users orders' do
    ...
    custom_results = User.select("users.name, count(orders.user_id) AS total_order_count").joins(:orders).group(:name).order("total_order_count DESC")
